...
TEST_CASE("Lexer recognizes all keywords", "[lexer][keywords]")
{
    ...
    REQUIRE(tokens.size() == 19); // 18 keywords + EOF
    ...
    CHECK(tokens[17].type == TokenType::KW_CHAR);
    CHECK(tokens[18].type == TokenType::END_OF_FILE);
}
...
TEST_CASE("Lexer tracks line numbers correctly", "[lexer][location]")
{
    ...
    for (const auto &token : tokens)
    {
        if (token.location.line == 1)
            line1Count++;
        else if (token.location.line == 2)
            line2Count++;
        else if (token.location.line == 3)
            line3Count++;
    }
...
TEST_CASE("Lexer tokenizes function declaration", "[lexer][integration]")
{
    ...
    REQUIRE(tokens.size() == 21); // Count all tokens including braces, semicolon, and EOF
