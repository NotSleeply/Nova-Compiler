...
inline Token Lexer::scanIdentifier() {
    ...
    if (it != KEYWORDS.end()) {
        ...
        if (it->second == TokenType::KW_TRUE) {
            return Token(TokenType::BOOL_LITERAL, true, loc, lexeme);
        } else if (it->second == TokenType::KW_FALSE) {
            return Token(TokenType::BOOL_LITERAL, false, loc, lexeme);
        }
...
inline Token Lexer::scanOperator() {
    ...
    switch (c) {
        ...
        case '(': return Token(TokenType::LPAREN, std::monostate{}, loc, "(");
        case ')': return Token(TokenType::RPAREN, std::monostate{}, loc, ")");
        case '{': return Token(TokenType::LBRACE, std::monostate{}, loc, "{");
        case '}': return Token(TokenType::RBRACE, std::monostate{}, loc, "}");
        case '[': return Token(TokenType::LBRACKET, std::monostate{}, loc, "[");
        case ']': return Token(TokenType::RBRACKET, std::monostate{}, loc, "]");
        case ',': return Token(TokenType::COMMA, std::monostate{}, loc, ",");
        case ';': return Token(TokenType::SEMICOLON, std::monostate{}, loc, ";");
        case '.': return Token(TokenType::DOT, std::monostate{}, loc, ".");
        
        // Potential multi-character operators
        case '+':
            if (match('+')) return Token(TokenType::OP_PLUS_PLUS, std::monostate{}, loc, "++");
            if (match('=')) return Token(TokenType::OP_PLUS_EQUAL, std::monostate{}, loc, "+=");
            return Token(TokenType::OP_PLUS, std::monostate{}, loc, "+");
            
        case '-':
            if (match('-')) return Token(TokenType::OP_MINUS_MINUS, std::monostate{}, loc, "--");
            if (match('=')) return Token(TokenType::OP_MINUS_EQUAL, std::monostate{}, loc, "-=");
            if (match('>')) return Token(TokenType::ARROW, std::monostate{}, loc, "->");
            return Token(TokenType::OP_MINUS, std::monostate{}, loc, "-");
            
        case '*':
            if (match('*')) return Token(TokenType::OP_STAR_STAR, std::monostate{}, loc, "**");
            if (match('=')) return Token(TokenType::OP_STAR_EQUAL, std::monostate{}, loc, "*=");
            return Token(TokenType::OP_STAR, std::monostate{}, loc, "*");
            
        case '/':
            if (match('=')) return Token(TokenType::OP_SLASH_EQUAL, std::monostate{}, loc, "/=");
            return Token(TokenType::OP_SLASH, std::monostate{}, loc, "/");
            
        case '%':
            if (match('=')) return Token(TokenType::OP_PERCENT_EQUAL, std::monostate{}, loc, "%=");
            return Token(TokenType::OP_PERCENT, std::monostate{}, loc, "%");
            
        case '=':
            if (match('=')) return Token(TokenType::OP_EQUAL_EQUAL, std::monostate{}, loc, "==");
            if (match('>')) return Token(TokenType::FAT_ARROW, std::monostate{}, loc, "=>");
            return Token(TokenType::OP_EQUAL, std::monostate{}, loc, "=");
            
        case '!':
            if (match('=')) return Token(TokenType::OP_BANG_EQUAL, std::monostate{}, loc, "!=");
            return Token(TokenType::OP_BANG, std::monostate{}, loc, "!");
            
        case '<':
            if (match('<')) return Token(TokenType::OP_LESS_LESS, std::monostate{}, loc, "<<");
            if (match('=')) return Token(TokenType::OP_LESS_EQUAL, std::monostate{}, loc, "<=");
            return Token(TokenType::OP_LESS, std::monostate{}, loc, "<");
            
        case '>':
            if (match('>')) return Token(TokenType::OP_GREATER_GREATER, std::monostate{}, loc, ">>");
            if (match('=')) return Token(TokenType::OP_GREATER_EQUAL, std::monostate{}, loc, ">=");
            return Token(TokenType::OP_GREATER, std::monostate{}, loc, ">");
            
        case '&':
            if (match('&')) return Token(TokenType::OP_AND, std::monostate{}, loc, "&&");
            return Token(TokenType::OP_AMPERSAND, std::monostate{}, loc, "&");
            
        case '|':
            if (match('|')) return Token(TokenType::OP_OR, std::monostate{}, loc, "||");
            return Token(TokenType::OP_PIPE, std::monostate{}, loc, "|");
            
        case '^': return Token(TokenType::OP_CARET, std::monostate{}, loc, "^");
        case '~': return Token(TokenType::OP_TILDE, std::monostate{}, loc, "~");
        
        case ':':
            if (match(':')) return Token(TokenType::COLON_COLON, std::monostate{}, loc, "::");
            return Token(TokenType::COLON, std::monostate{}, loc, ":");
            
        default:
            return Token(TokenType::UNKNOWN, std::monostate{}, loc, std::string(1, c));
    }
}
